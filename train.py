...
class ClipCocoDataset(Dataset):
    def __init__(self, data_path: str, prefix_length: int = 10, 
                 ...
                 subset_size: int = None, max_seq_len: Optional[int] = None):  # Add max_seq_len parameter
        ...
        if subset_size is not None:
            ...
            self.clip_embeddings = {k: v for k, v in data["clip_embedding"].items() if k in img_ids}
        else:
            self.clip_embeddings = data["clip_embedding"]
            self.captions = data["captions"]
            self.captions_tokens = data["tokens"]
        
        # Use provided max_seq_len or calculate per-dataset
        self.max_seq_len = max_seq_len if max_seq_len is not None else self._calculate_max_len()
    
    def _calculate_max_len(self):
        all_len = torch.tensor([len(t) for t in self.captions_tokens]).float()
        return min(int(all_len.mean() + all_len.std() * 10), int(all_len.max()))
...
def calculate_global_max_len(dataset_paths, subset_size=None):
    all_lengths = []
    for path in dataset_paths:
        with open(path, 'rb') as f:
            data = pickle.load(f)
        tokens = data["tokens"][:subset_size] if subset_size else data["tokens"]
        all_lengths.extend([len(t) for t in tokens])
    all_len = torch.tensor(all_lengths).float()
    return min(int(all_len.mean() + all_len.std() * 10), int(all_len.max()))
...
def train(dataset: ClipCocoDataset, model: CLIPPrefixCaption, args, output_dir: str = ".", output_prefix: str = ""):
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, clip_embed) in enumerate(train_dataloader):
            ...
            logits = outputs.logits[:, args.prefix_length-1:-1]  # Exclude prefix/last token
        ...
        # Save epoch checkpoint (will overwrite if exists)
        epoch_ckpt = os.path.join(output_dir, f"epoch_{epoch}.pt")
        torch.save(
            model.state_dict(),
            epoch_ckpt,
            _use_new_zipfile_serialization=True
        )
        print(f"Saved checkpoint: {epoch_ckpt} | Avg Loss: {epoch_loss/len(train_dataloader):.4f}")
    
    # Final save (overwrites existing final weights)
    final_ckpt = os.path.join(output_dir, "transformer_mapper_weights.pt")
    torch.save(
        model.state_dict(),
        final_ckpt,
        _use_new_zipfile_serialization=True
...
def main():
    # Datasets
    datasets_to_train = [
        "F:/coco_pickle2014/train2014_processed.pkl",
        "F:/coco_pickle2017/train2017_processed.pkl", 
        "F:/coco_pickle2017/val2017_processed.pkl"
    ]
    ...
    global_max_len = calculate_global_max_len(datasets_to_train, args.subset_size)  
    ...
    datasets = [
        ClipCocoDataset(
            ...
            max_seq_len=global_max_len  # Pass precomputed value
        ) for path in datasets_to_train
    ]
    
    # Calculate inverse-size weights
    dataset_sizes = [len(d) for d in datasets]
    weights = [1.0 / size for size in dataset_sizes]
    sample_weights = torch.cat([torch.full((len(d),), w) for d, w in zip(datasets, weights)])
    
    # Create combined dataset with sampler
    combined_dataset = ConcatDataset(datasets)
    combined_dataset.sampler = WeightedRandomSampler(
        sample_weights,
        len(sample_weights),
        replacement=True
    )
    
    train(combined_dataset, model, args, args.output_dir, "Training")
